            ...
            {isOpen &&
                <div className="fixed z-10 top-0 left-0 w-full h-full bg-black/50 backdrop-blur-sm">
                    <div className="relative z-20 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-h-[95vh] md:max-h-[90vh] w-[97vw] md:w-[80vw] p-4 bg-slate-100 shadow-lg rounded-xl overflow-auto" ref={modalRef}>
                        <CommentModalContent data={data} setIsOpenAction={setIsOpenAction} />
                    </div>
                </div>
            }
        </>
...
// コメント用モーダルの中身
export function CommentModalContent({ data, setIsOpenAction }: { data?: { postId: number, userId: number }, setIsOpenAction: React.Dispatch<React.SetStateAction<boolean>> }) {
    ...
    const [state, commentAction, isPending] = useActionState(
        async (_prevState: any, formData: FormData) => {
            const comment = addComment(_prevState, formData, data!.postId, data!.userId);
            setIsOpenAction(false);
            router.refresh();
            return comment;
        }, null);
